# TODO: Translation updated at 2022-07-30 19:01

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:36
translate chinese_simplified dojo_exterior_292117b8:

    # player "wow ! Impressionnant..." id dojo_exterior_292117b8
    player "哇！好壮观..." id dojo_exterior_292117b8

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:37
translate chinese_simplified dojo_exterior_dd982f53:

    # player "Rien qu'à l'entrée, on est déjà bien dans l'ambiance." id dojo_exterior_dd982f53
    player "刚到门口，我感觉就充满了力量。" id dojo_exterior_dd982f53

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:38
translate chinese_simplified dojo_exterior_e31bf590:

    # player "Autant aller jeter un coup d'œil à l'intérieur." id dojo_exterior_e31bf590
    player "不妨进去看看。" id dojo_exterior_e31bf590

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:40
translate chinese_simplified dojo_exterior_e8e15207:

    # akirac "STOP !" id dojo_exterior_e8e15207
    akirac "站住 ！" id dojo_exterior_e8e15207

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:41
translate chinese_simplified dojo_exterior_3cd69d40:

    # player -happy "...!?" id dojo_exterior_3cd69d40
    player -happy "...！？" id dojo_exterior_3cd69d40

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:46
translate chinese_simplified dojo_exterior_8214f5e8:

    # akira upset "N'espère même pas faire un seul pas de plus, déchet !" id dojo_exterior_8214f5e8
    akira upset "别再往前走了，臭小子！" id dojo_exterior_8214f5e8

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:47
translate chinese_simplified dojo_exterior_b10797b8:

    # player upset "Quoi?!" id dojo_exterior_b10797b8
    player upset "什么？！" id dojo_exterior_b10797b8

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:48
translate chinese_simplified dojo_exterior_a61d087c:

    # akira -upset "Oui couille molle ! En plus de ça, t'es sourd ?" id dojo_exterior_a61d087c
    akira -upset "我在和你说话，软蛋！你是聋子吗？" id dojo_exterior_a61d087c

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:49
translate chinese_simplified dojo_exterior_c9b0020c:

    # akira "T'as décidément rien pour toi..." id dojo_exterior_c9b0020c
    akira "天啊，你看起来瘦不拉几的..." id dojo_exterior_c9b0020c

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:55
translate chinese_simplified dojo_exterior_113a5263:

    # x "Putain...{color=#fdcb6e}{b}[akira_name]{/b}{/color}... Je l'avais oublié celui-là..." id dojo_exterior_113a5263
    x "该死...{color=#fdcb6e}{b}明{/b}{/color}...我差点把他给忘了..." id dojo_exterior_113a5263

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:56
translate chinese_simplified dojo_exterior_f74cdb95:

    # x "C'était et c'est toujours la pire ordure de toute mon adolescence..." id dojo_exterior_f74cdb95
    x "它曾经、现在仍然是我见过的最糟糕的人渣..." id dojo_exterior_f74cdb95

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:57
translate chinese_simplified dojo_exterior_b3e9ba34:

    # x "Toujours là avec ses insultes, ses humiliations et sa violence gratuite." id dojo_exterior_b3e9ba34
    x "他的出现总是伴随着侮辱、羞辱和无端的暴力。" id dojo_exterior_b3e9ba34

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:58
translate chinese_simplified dojo_exterior_a9552264:

    # x "Finalement c'est un peu une version masculine de {color=#fd79a8}{b}纪子{/b}{/color}" id dojo_exterior_a9552264
    x "他就像是 {color=#fd79a8}{b}纪子{/b}{/color}，但是长了个屌" id dojo_exterior_a9552264

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:59
translate chinese_simplified dojo_exterior_dfdeb035:

    # x "Je n'avais aucune idée qu'il pratiquait un sport de combat lui aussi..." id dojo_exterior_dfdeb035
    x "但我不知道他也在练习格斗..." id dojo_exterior_dfdeb035

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:60
translate chinese_simplified dojo_exterior_3dca99e9:

    # x "Fait chié, c'était pas prévu du tout ça." id dojo_exterior_3dca99e9
    x "该死，这根本都是计划外的状况。" id dojo_exterior_3dca99e9

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:67
translate chinese_simplified dojo_exterior_d75febc6:

    # akira "Oh la vache, il répond...!" id dojo_exterior_d75febc6
    akira "哦看啊，这个傻吊说话了...！" id dojo_exterior_d75febc6

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:68
translate chinese_simplified dojo_exterior_1ddd0f53:

    # akira "Et bien alors, on s'est acheté des couilles ?" id dojo_exterior_1ddd0f53
    akira "你妈妈给你买了新蛋啦？" id dojo_exterior_1ddd0f53

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:69
translate chinese_simplified dojo_exterior_cc68c0ea:

    # akira "Visiblement c'était le plus petit modèle..." id dojo_exterior_cc68c0ea
    akira "挑了什么尺寸给你啊？小？还是最小？..." id dojo_exterior_cc68c0ea

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:71
translate chinese_simplified dojo_exterior_92cfab26:

    # akira "Va te suicider et tu n'entendras plus parler de moi !" id dojo_exterior_92cfab26
    akira "去自杀吧，你不会再听到我的消息了！" id dojo_exterior_92cfab26

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:72
translate chinese_simplified dojo_exterior_03775ec8:

    # player "..." id dojo_exterior_03775ec8
    player "..." id dojo_exterior_03775ec8

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:73
translate chinese_simplified dojo_exterior_f28cd992:

    # player neutral "Je veux seulement m'inscrire..." id dojo_exterior_f28cd992
    player neutral "我只是想来报名而已..." id dojo_exterior_f28cd992

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:74
translate chinese_simplified dojo_exterior_96f5c8aa:

    # akira neutral "J'espère que tu t'imagines pas venir ici et apprendre à te battre pour essayer de te venger?" id dojo_exterior_96f5c8aa
    akira neutral "哦？臭小鬼想来这里练武报仇？" id dojo_exterior_96f5c8aa

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:75
translate chinese_simplified dojo_exterior_03775ec8_1:

    # player "..." id dojo_exterior_03775ec8_1
    player "..." id dojo_exterior_03775ec8_1

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:76
translate chinese_simplified dojo_exterior_6aec3ddd:

    # akira arrogant "Mon pauvre, mais tu fais tellement peine... Tu aurais dû t'y mettre y'a des années pour pouvoir faire quelque chose aujourd'hui." id dojo_exterior_6aec3ddd
    akira arrogant "可怜的孩子，你应该从几年前开始练起。" id dojo_exterior_6aec3ddd

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:77
translate chinese_simplified dojo_exterior_53f8c629:

    # akira "C'est trop tard pour toi." id dojo_exterior_53f8c629
    akira "现在对你来说已经太晚了。" id dojo_exterior_53f8c629

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:78
translate chinese_simplified dojo_exterior_d35c3e62:

    # akira "T'es juste un putain de cas désespéré !" id dojo_exterior_d35c3e62
    akira "完全没有希望！" id dojo_exterior_d35c3e62

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:79
translate chinese_simplified dojo_exterior_b073c95d:

    # akira angry "Maintenant, casse-toi ou je te jure que je te démonte !" id dojo_exterior_b073c95d
    akira angry "现在给我滚开，否则我发誓我会杀了你！" id dojo_exterior_b073c95d

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:83
translate chinese_simplified dojo_exterior_45b08d41:

    # player upset "Quelle espèce de gros bâtard..." id dojo_exterior_45b08d41
    player upset "什么傻逼..." id dojo_exterior_45b08d41

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:86
translate chinese_simplified dojo_exterior_f5ef3a00:

    # akira "Je crois que j'ai mal entendu, mais tu ne viendrais pas de m'insulter là ?" id dojo_exterior_f5ef3a00
    akira "我想我听错了，你在那嘀咕什么呢？" id dojo_exterior_f5ef3a00

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:87
translate chinese_simplified dojo_exterior_c2cffe85:

    # player upset "..." id dojo_exterior_c2cffe85
    player upset "..." id dojo_exterior_c2cffe85

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:88
translate chinese_simplified dojo_exterior_99ab2e0d:

    # akira "Non parce que si tu cherches a mourir dis le tout de suite !" id dojo_exterior_99ab2e0d
    akira "如果你想死，说一声，我送你上路！" id dojo_exterior_99ab2e0d

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:89
translate chinese_simplified dojo_exterior_3b492d5b:

    # akira "Je vais te faire gagner du temps." id dojo_exterior_3b492d5b
    akira "我会慢慢折磨你。" id dojo_exterior_3b492d5b

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:90
translate chinese_simplified dojo_exterior_43cb2245:

    # akira "Barre-toi jte dis !" id dojo_exterior_43cb2245
    akira "保证让你痛不欲生！" id dojo_exterior_43cb2245

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:92
translate chinese_simplified dojo_exterior_c8532248:

    # akira arrogant "Quelle putain de cassos..." id dojo_exterior_c8532248
    akira arrogant "什么弱智..." id dojo_exterior_c8532248

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:96
translate chinese_simplified dojo_exterior_dd842e49:

    # player "Putain !" id dojo_exterior_dd842e49
    player "操！" id dojo_exterior_dd842e49

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:100
translate chinese_simplified dojo_exterior_3f45257a:

    # x "Bon... Si je résume..." id dojo_exterior_3f45257a
    x "嗯...让我总结一下..." id dojo_exterior_3f45257a

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:101
translate chinese_simplified dojo_exterior_873b55fb:

    # x "Je n'ai pas pu courir ce matin." id dojo_exterior_873b55fb
    x "今天早上步没跑成。" id dojo_exterior_873b55fb

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:102
translate chinese_simplified dojo_exterior_2d3f3716:

    # x "Impossible de travailler." id dojo_exterior_2d3f3716
    x "工作没找到。" id dojo_exterior_2d3f3716

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:103
translate chinese_simplified dojo_exterior_2709905a:

    # x "Aucune chance d'accéder au dojo..." id dojo_exterior_2709905a
    x "道场还他妈被恶棍挡着......" id dojo_exterior_2709905a

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:104
translate chinese_simplified dojo_exterior_a21e86ee:

    # x "..." id dojo_exterior_a21e86ee
    x "..." id dojo_exterior_a21e86ee

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:105
translate chinese_simplified dojo_exterior_da661e84:

    # x "Une belle grosse journée de merde!" id dojo_exterior_da661e84
    x "糟糕的一天！" id dojo_exterior_da661e84

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:106
translate chinese_simplified dojo_exterior_a4c6dea4:

    # x "Tant pis pour aujourd'hui, je réfléchirai à tout ça demain..." id dojo_exterior_a4c6dea4
    x "今天太糟糕了，但事情总会好起来的......" id dojo_exterior_a4c6dea4

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:126
translate chinese_simplified dojo_exterior_73a3cff0:

    # x "{color=#ffcccc}{b}[chiaki_name]{/b}{/color} n'est pas là." id dojo_exterior_73a3cff0
    x "{color=#ffcccc}{b}千秋{/b}{/color} 不在这里。" id dojo_exterior_73a3cff0

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:127
translate chinese_simplified dojo_exterior_aee6a768:

    # x "En même temps qu'est-ce qu'elle viendrait faire au dojo." id dojo_exterior_aee6a768
    x "也对，她来道场做什么？" id dojo_exterior_aee6a768

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:128
translate chinese_simplified dojo_exterior_69a49149:

    # x "Cette histoire est quand même sacrement louche." id dojo_exterior_69a49149
    x "这个故事还是很坑爹的。" id dojo_exterior_69a49149

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:129
translate chinese_simplified dojo_exterior_9ec986cf:

    # x "Même si {color=#e17055}{b}水人{/b}{/color} est un peu surprotecteur avec sa sœur pourquoi chercherait-elle à ce point a fuir à la moindre occasion." id dojo_exterior_9ec986cf
    x "就算{color=#e17055}{b}水人{/b}{/color}对自己的妹妹有点过分保护，她又何必一有机会就这么拼命逃跑。" id dojo_exterior_9ec986cf

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:130
translate chinese_simplified dojo_exterior_56020f67:

    # x "Surtout avec cet air terrifié." id dojo_exterior_56020f67
    x "尤其是那惊恐的表情。" id dojo_exterior_56020f67

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:131
translate chinese_simplified dojo_exterior_826a4c17:

    # x "Je me fais peut être des idées..." id dojo_exterior_826a4c17
    x "但仔细一想..." id dojo_exterior_826a4c17

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:132
translate chinese_simplified dojo_exterior_36bc5190:

    # x "Et puis c'est qui ce soi-disant mec qui lui tourne autour?" id dojo_exterior_36bc5190
    x "那个所谓的跟踪狂又是谁呢？" id dojo_exterior_36bc5190

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:137
translate chinese_simplified dojo_exterior_82627d98:

    # player "Je devrais essayer d'aller voir ailleurs." id dojo_exterior_82627d98
    player "我应该试着去别处看看。" id dojo_exterior_82627d98

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:138
translate chinese_simplified dojo_exterior_3a353e62:

    # player "Je finirais bien par tomber sur elle." id dojo_exterior_3a353e62
    player "我最终会遇到她。" id dojo_exterior_3a353e62

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:143
translate chinese_simplified dojo_exterior_893d3010:

    # player "Oh! Maintenant que j'y pense, je n'ai pas encore essayé le konbini!" id dojo_exterior_893d3010
    player "哦！想想看，我还没试过便利店呢！" id dojo_exterior_893d3010

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:144
translate chinese_simplified dojo_exterior_dd1ea67d:

    # player happy "Je crois me souvenir qu'ils y vendent des glaces..." id dojo_exterior_dd1ea67d
    player happy "我好像记得他们在那里卖冰淇淋..." id dojo_exterior_dd1ea67d

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:145
translate chinese_simplified dojo_exterior_2213e36d:

    # player "Elle sera forcement là-bas!" id dojo_exterior_2213e36d
    player "她一定会在那里！" id dojo_exterior_2213e36d

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:157
translate chinese_simplified dojo_exterior_3eb73dbd:

    # playerc "Arrête toi qu'on discute!" id dojo_exterior_3eb73dbd
    playerc "给我站住！" id dojo_exterior_3eb73dbd

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:160
translate chinese_simplified dojo_exterior_bfdd1eac:

    # kimochi "Je suis hétéro désolé!" id dojo_exterior_bfdd1eac
    kimochi "我身上没有急支糖浆！" id dojo_exterior_bfdd1eac

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:163
translate chinese_simplified dojo_exterior_5f6f4b02:

    # player sad "Comment fait-il pour courir aussi vite?" id dojo_exterior_5f6f4b02
    player sad "他怎么跑得这么快？" id dojo_exterior_5f6f4b02

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:164
translate chinese_simplified dojo_exterior_4d60973b:

    # player "Il se dirige vers le {color=#ff0}{b}parc{/b}{/color} à présent." id dojo_exterior_4d60973b
    player "他现在要去 {color=#ff0}{b}公园{/b}{/color}。" id dojo_exterior_4d60973b

translate chinese_simplified strings:

    # game/scripts/story/ancient district/dojo/dojo_exterior.rpy:28
    old "Autant rentrer à la {color=ffff00}{b}maison{/b}{/color}..."
    new "还是回 {color=ffff00}{b}家{/b}{/color} 吧..."

    # game/scripts/story/ancient district/dojo/dojo_exterior.rpy:65
    old "Qu'est-ce que tu me veux ?"
    new "你想干嘛？"

    # game/scripts/story/ancient district/dojo/dojo_exterior.rpy:65
    old "Laisse-moi tranquille..."
    new "离我远点..."

# TODO: Translation updated at 2022-12-15 12:14

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:173
translate chinese_simplified dojo_exterior_24f5eff2:

    # akira "Directement dans la gueule du loup." id dojo_exterior_24f5eff2
    akira "就在狼嘴里。" id dojo_exterior_24f5eff2

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:174
translate chinese_simplified dojo_exterior_0f298bc3:

    # player furious "Mais putain lachez moi!" id dojo_exterior_0f298bc3
    player furious "放开我！" id dojo_exterior_0f298bc3

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:175
translate chinese_simplified dojo_exterior_58ec0130:

    # akira angry "Seulement quand tu seras mort!" id dojo_exterior_58ec0130
    akira angry "只有当你死了！" id dojo_exterior_58ec0130

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:191
translate chinese_simplified dojo_exterior_806d4cb0:

    # akira "Monsieur [player.n] nous daigne enfin de sa présence." id dojo_exterior_806d4cb0
    akira "[player.n]先生终于屈尊让我们看到他了。" id dojo_exterior_806d4cb0

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:192
translate chinese_simplified dojo_exterior_33854b34:

    # akira "Je suppose que tu sais pourquoi tu là?" id dojo_exterior_33854b34
    akira "我想你知道你为什么在这里？" id dojo_exterior_33854b34

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:193
translate chinese_simplified dojo_exterior_7ef10cbb:

    # player "Parce que tu t'es fais rejeté par une fille et que tu cherches a compenser ton ego blésssé?" id dojo_exterior_7ef10cbb
    player "因为你被一个女孩拒绝了，想弥补你的自负？" id dojo_exterior_7ef10cbb

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:195
translate chinese_simplified dojo_exterior_3070a588:

    # kenji "Ooooh, elle a bien grandit notre victime préféré." id dojo_exterior_3070a588
    kenji "哦，她长大了，我们最喜欢的受害者。" id dojo_exterior_3070a588

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:196
translate chinese_simplified dojo_exterior_ebf30710:

    # takeo "C'est clair, il a du se faire pousser quelques testicules supplémentaires!" id dojo_exterior_ebf30710
    takeo "很明显，他一定有多余的睾丸！" id dojo_exterior_ebf30710

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:197
translate chinese_simplified dojo_exterior_d32f578d:

    # akira "Fermez vos gueules!" id dojo_exterior_d32f578d
    akira "闭上你的嘴！" id dojo_exterior_d32f578d

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:198
translate chinese_simplified dojo_exterior_6eeb3e27:

    # akira "Je ne sais pas ce qu'il s'est passé pour que tu changes autant en quelque temps." id dojo_exterior_6eeb3e27
    akira "我不知道发生了什么事让你在一段时间内改变了这么多。" id dojo_exterior_6eeb3e27

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:199
translate chinese_simplified dojo_exterior_93e30adc:

    # akira "Mais je vais te montrer que tout ça n'a servis a rien." id dojo_exterior_93e30adc
    akira "但我要告诉你这一切都没用。" id dojo_exterior_93e30adc

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:200
translate chinese_simplified dojo_exterior_18a22283:

    # akira "Sauf a aggraver ton cas!" id dojo_exterior_18a22283
    akira "除了让你更糟！" id dojo_exterior_18a22283

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:201
translate chinese_simplified dojo_exterior_b47a3411:

    # player "Si vous voulez vraiment m'arretez faudra me tuer!" id dojo_exterior_b47a3411
    player "如果你真的想阻止我，你必须杀了我！" id dojo_exterior_b47a3411

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:202
translate chinese_simplified dojo_exterior_e8b68a52:

    # akira neutral "..." id dojo_exterior_e8b68a52
    akira neutral "..." id dojo_exterior_e8b68a52

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:203
translate chinese_simplified dojo_exterior_a6c2a972:

    # kenji "{size=-10}Putain il est chaud boullant.{/size}" id dojo_exterior_a6c2a972
    kenji "{size=-10}他妈的热得要命。{/size}" id dojo_exterior_a6c2a972

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:204
translate chinese_simplified dojo_exterior_f3694c1d:

    # takeo "{size=-10}Mais ouais, j'en ai des frissons.{/size}" id dojo_exterior_f3694c1d
    takeo "{size=-10}是的，我很紧张。{/size}" id dojo_exterior_f3694c1d

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:205
translate chinese_simplified dojo_exterior_e415bcd7:

    # akira arrogant "C'est déja ce que j'avais en tête!" id dojo_exterior_e415bcd7
    akira arrogant "这就是我的想法！" id dojo_exterior_e415bcd7

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:206
translate chinese_simplified dojo_exterior_4596023f:

    # nobu disgusted "euh... on avait dit qu'on le tabassait mais qu'on allait le tuer." id dojo_exterior_4596023f
    nobu disgusted "呃…我们说我们要揍他，但我们要杀了他。" id dojo_exterior_4596023f

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:207
translate chinese_simplified dojo_exterior_a8acba43:

    # kenji disgusted "Je suis pas très chaud non plus, on nous a vu courir avec lui partout dans la ville." id dojo_exterior_a8acba43
    kenji disgusted "我也不太暖和，有人看见我们和他在城里到处跑。" id dojo_exterior_a8acba43

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:208
translate chinese_simplified dojo_exterior_1050cd30:

    # takeo disgusted "La même, j'ai pas envie de passer ma vie en prison." id dojo_exterior_1050cd30
    takeo disgusted "我也不想在监狱里度过余生。" id dojo_exterior_1050cd30

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:209
translate chinese_simplified dojo_exterior_1c01737a:

    # akira "On aura des problemes seulement si on retrouve le corps." id dojo_exterior_1c01737a
    akira "只有找到尸体我们才会有麻烦。" id dojo_exterior_1c01737a

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:210
translate chinese_simplified dojo_exterior_97c342fd:

    # akira "Je connais un endroit ou il ne sera jamais retrouvé." id dojo_exterior_97c342fd
    akira "我知道一个永远找不到的地方。" id dojo_exterior_97c342fd

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:217
translate chinese_simplified dojo_exterior_d28302ea:

    # x "Pourquoi j'ai l'impression qu'il est vraiment serieux." id dojo_exterior_d28302ea
    x "为什么我觉得他很认真。" id dojo_exterior_d28302ea

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:218
translate chinese_simplified dojo_exterior_ba0fbb3f:

    # x "Je ne pensais pas qu'il irait jusqu'ici." id dojo_exterior_ba0fbb3f
    x "我没想到他会走这么远。" id dojo_exterior_ba0fbb3f

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:219
translate chinese_simplified dojo_exterior_f43de213:

    # x "Maintenant que j'y pense est ce que c'est lui s'est débarassé de [noriko.n] dans ma précédente vie?" id dojo_exterior_f43de213
    x "现在我想起来了，是他在我以前的生活中把自己从[noriko.n]中抹去了吗？" id dojo_exterior_f43de213

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:220
translate chinese_simplified dojo_exterior_86158feb:

    # x "Si je ne fais rien je vais subir le même sort." id dojo_exterior_86158feb
    x "如果我不做任何事，我也会经历同样的命运。" id dojo_exterior_86158feb

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:221
translate chinese_simplified dojo_exterior_7253c3bc:

    # x "Il va me falloir un miracle pour me sortir de ce pétrin." id dojo_exterior_7253c3bc
    x "我需要一个奇迹才能摆脱困境。" id dojo_exterior_7253c3bc

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:228
translate chinese_simplified dojo_exterior_bb6a42e4:

    # akira "Tenez le bien!" id dojo_exterior_bb6a42e4
    akira "抓住他！" id dojo_exterior_bb6a42e4

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:229
translate chinese_simplified dojo_exterior_d63d6b65:

    # takeo "..." id dojo_exterior_d63d6b65
    takeo "..." id dojo_exterior_d63d6b65

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:230
translate chinese_simplified dojo_exterior_4e85f34e:

    # kenji "Putain on va droit vers les emmerdes." id dojo_exterior_4e85f34e
    kenji "我们他妈的要惹麻烦了。" id dojo_exterior_4e85f34e

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:231
translate chinese_simplified dojo_exterior_ddb769c5:

    # akira angry "DEPECHEZ VOUS BORDEL!" id dojo_exterior_ddb769c5
    akira angry "他妈的滚出去！" id dojo_exterior_ddb769c5

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:232
translate chinese_simplified dojo_exterior_35251e2c:

    # inconnus "HEY! VOUS!" id dojo_exterior_35251e2c
    inconnus "嘿！嘿！你！" id dojo_exterior_35251e2c

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:233
translate chinese_simplified dojo_exterior_b39d2e22:

    # nobu neutral "On nous a vous barrons nous!" id dojo_exterior_b39d2e22
    nobu neutral "我们抓到你了！" id dojo_exterior_b39d2e22

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:234
translate chinese_simplified dojo_exterior_975a6399:

    # akira "REVENEZ BANDE DE L-" id dojo_exterior_975a6399
    akira "回来，伙计们-" id dojo_exterior_975a6399

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:243
translate chinese_simplified dojo_exterior_6f1d2d0f:

    # ubs "Vous n'allez rien faire du tout bande de petits cons." id dojo_exterior_6f1d2d0f
    ubs "你们这些小混蛋什么都不会做。" id dojo_exterior_6f1d2d0f

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:244
translate chinese_simplified dojo_exterior_1b30a65e:

    # ubs bad "J'ai filmé toute la scene, s'il arrive quoi que se soit a ce garçon je vais directement voir la police." id dojo_exterior_1b30a65e
    ubs bad "我拍了整个场景，如果这个男孩出了什么事，我会直接报警的。" id dojo_exterior_1b30a65e

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:245
translate chinese_simplified dojo_exterior_e8b68a52_1:

    # akira neutral "..." id dojo_exterior_e8b68a52_1
    akira neutral "..." id dojo_exterior_e8b68a52_1

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:246
translate chinese_simplified dojo_exterior_5fefe4f3:

    # akira "Vous ne savez pas a qui vous avez affaire!" id dojo_exterior_5fefe4f3
    akira "你不知道你在和谁打交道！" id dojo_exterior_5fefe4f3

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:247
translate chinese_simplified dojo_exterior_5fcfcb0e:

    # ubs "Ouai c'est ce que ta mere m'a dit hier soir avant de me sucer." id dojo_exterior_5fcfcb0e
    ubs "是啊，你妈妈昨晚吸我之前就是这么说的。" id dojo_exterior_5fcfcb0e

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:248
translate chinese_simplified dojo_exterior_6517777e:

    # ubs "Et elle avait raison, je ne m'attendais pas me retrouver avec la meilleure salope du quartier!" id dojo_exterior_6517777e
    ubs "她是对的，我没想到我会和附近最好的婊子在一起！" id dojo_exterior_6517777e

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:250
translate chinese_simplified dojo_exterior_fc2622b7:

    # akira angry "...Faites attention a ce que vous dites, votre boutique pourrait prendre feu, un accident est vité arrivé!" id dojo_exterior_fc2622b7
    akira angry "…小心你说的话，你的商店可能着火了，事故发生了！" id dojo_exterior_fc2622b7

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:251
translate chinese_simplified dojo_exterior_5b5bd7a1:

    # ubs "Oups désolé j'avais oublié d'arreter l'enregistrement." id dojo_exterior_5b5bd7a1
    ubs "哦，对不起，我忘了停止录音。" id dojo_exterior_5b5bd7a1

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:252
translate chinese_simplified dojo_exterior_e6b454b9:

    # ubs "Tu disais?" id dojo_exterior_e6b454b9
    ubs "你说什么？" id dojo_exterior_e6b454b9

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:253
translate chinese_simplified dojo_exterior_01172fe6:

    # akira "Putain!" id dojo_exterior_01172fe6
    akira "他妈的！" id dojo_exterior_01172fe6

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:254
translate chinese_simplified dojo_exterior_aa36628e:

    # akira "Ca se terminera pas comme ça!" id dojo_exterior_aa36628e
    akira "不会这样结束的！" id dojo_exterior_aa36628e

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:255
translate chinese_simplified dojo_exterior_ae074b7e:

    # akira "Et toi le dechet... Je te conseille de raser les murs!" id dojo_exterior_ae074b7e
    akira "我建议你把墙夷为平地。" id dojo_exterior_ae074b7e

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:256
translate chinese_simplified dojo_exterior_22c611b3:

    # ubs angry "Casse toi maintenant!" id dojo_exterior_22c611b3
    ubs angry "快离开这里！" id dojo_exterior_22c611b3

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:257
translate chinese_simplified dojo_exterior_a412d196:

    # akira "Tch" id dojo_exterior_a412d196
    akira "C" id dojo_exterior_a412d196

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:262
translate chinese_simplified dojo_exterior_2a72d32b:

    # ubs "Et bien... Tu tes vraiment mis dans un sacré pétrin." id dojo_exterior_2a72d32b
    ubs "好吧…你真是一团糟。" id dojo_exterior_2a72d32b

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:263
translate chinese_simplified dojo_exterior_ef3f6652:

    # ubs "Ca me rapelle un peu ma jeunesse." id dojo_exterior_ef3f6652
    ubs "这让我想起了我年轻的时候。" id dojo_exterior_ef3f6652

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:264
translate chinese_simplified dojo_exterior_91d1922b:

    # player "Pourquoi?{w} Pourquoi vous avez changé d'avis?" id dojo_exterior_91d1922b
    player "怎么了？{w}你为什么改变主意？" id dojo_exterior_91d1922b

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:265
translate chinese_simplified dojo_exterior_5146ba0a:

    # ubs arrogant "Quoi tu ne penses pas qu'un homme a le droit de regretter ses actes et de changer d'avis?" id dojo_exterior_5146ba0a
    ubs arrogant "你不认为一个人有权后悔和改变主意吗？" id dojo_exterior_5146ba0a

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:270
translate chinese_simplified dojo_exterior_9029b20b:

    # x "Merde j'ai beau savoir qu'il m'a aidé..." id dojo_exterior_9029b20b
    x "妈的，我知道他帮了我。。。" id dojo_exterior_9029b20b

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:271
translate chinese_simplified dojo_exterior_e48197db:

    # x "Je le sent vraiment pas." id dojo_exterior_e48197db
    x "我真的感觉不到。" id dojo_exterior_e48197db

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:277
translate chinese_simplified dojo_exterior_7e56af26:

    # ubs "Allez petit ingrat, je te raccompagne chez toi!" id dojo_exterior_7e56af26
    ubs "来吧，你这个忘恩负义的小家伙，我送你回家！" id dojo_exterior_7e56af26

# game/scripts/story/ancient district/dojo/dojo_exterior.rpy:278
translate chinese_simplified dojo_exterior_75f8a3db:

    # player "Comment vous savez ou j'habite?" id dojo_exterior_75f8a3db
    player "你怎么知道我住在哪里？" id dojo_exterior_75f8a3db

translate chinese_simplified strings:

    # game/scripts/story/ancient district/dojo/dojo_exterior.rpy:182
    old "Rentrez chez vous escorter par [ubs.n]"
    new "由[ubs.n]护送回家"

